...
async def demonstrate_streaming_logs():
    ...
    class MockJobClient:
        def get_job_logs(self, job_id):
            ...
            return "\n".join([f"Log line {i}: {large_line}" for i in range(20000)])  # ~20MB
    ...
    print(f"✅ Line limit enforced: {len(lines) == 11}")  # 10 lines + truncation message
    ...
    class VeryLargeMockJobClient:
        def get_job_logs(self, job_id):
            ...
            large_line = "x" * 1024  # 1KB per line
            return "\n".join([f"Log line {i}: {large_line}" for i in range(50000)])  # ~50MB
