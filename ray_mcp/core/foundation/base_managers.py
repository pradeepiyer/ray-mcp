"""Base manager classes to eliminate code duplication and enforce consistent patterns."""
...
class BaseManager(ABC):
    ...
    def __init__(self, state_manager: StateManager):
        ...
        # Single shared response formatter instance
    ...
    def _format_error_response(
        ...
        return self._response_formatter.format_error_response(operation, error)

    def _format_validation_error(self, message: str) -> Dict[str, Any]:
        """Format validation error response."""
        return self._response_formatter.format_validation_error(message)
...
class RayBaseManager(BaseManager):
    """Base class for Ray-related managers."""
    ...
    def _is_ray_initialized(self) -> bool:
        ...
        return (
            self._ray and self._ray.is_initialized() if self._RAY_AVAILABLE else False
        )


class KubernetesBaseManager(BaseManager):
    """Base class for Kubernetes-related managers."""

    def __init__(self, state_manager: StateManager):
        super().__init__(state_manager)

        # Get Kubernetes imports
...
class CloudProviderBaseManager(BaseManager):
    """Base class for cloud provider managers."""

    def __init__(self, state_manager: StateManager):
        super().__init__(state_manager)

        # Get Google Cloud imports
...
class KubeRayBaseManager(KubernetesBaseManager):
    """Base class for KubeRay-related managers."""
...
class StateManagementMixin:
    """Mixin for standardized state management patterns."""
...
class AsyncOperationMixin:
    """Mixin for standardized async operation patterns."""
