...
class CRDOperationsClient(CRDOperations):
    ...
    def _get_resource_info(self, resource_type: str) -> Dict[str, str]:
        ...
        if resource_type_lower not in self.RESOURCE_MAPPINGS:
            raise ValueError(f"Unsupported resource type: {resource_type}. Supported types: {list(self.RESOURCE_MAPPINGS.keys())}")
    ...
    async def get_resource(self, resource_type: str, name: str, namespace: str = "default") -> Dict[str, Any]:
        ...
        try:
            ...
            if e.status == 404:
                return self._response_formatter.format_error_response(
                    "get custom resource",
                    Exception(f"Resource '{name}' not found in namespace '{namespace}'")
                )
            else:
                return self._response_formatter.format_error_response(
                    "get custom resource",
    ...
    async def update_resource(self, resource_type: str, name: str, resource_spec: Dict[str, Any], namespace: str = "default") -> Dict[str, Any]:
        ...
        for attempt in range(self._retry_attempts):
            try:
                ...
                elif e.status == 409 and attempt < self._retry_attempts - 1:  # Conflict, retry
                    LoggingUtility.log_warning(
                        "update custom resource", 
