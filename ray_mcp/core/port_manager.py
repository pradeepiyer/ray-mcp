        ...
        for attempt in range(max_tries):
            if await self._try_allocate_port(port, temp_dir):
                LoggingUtility.log_info(
                    "port_allocation",
                    f"Successfully allocated port {port}"
                )
                return port
            port += 1
            ...
            with open(lock_file_path, "r") as f:
                ...
                if "," in content:
                    ...
                    try:
                        ...
                        return (current_time - timestamp) < 300  # Less than 5 minutes old
                    except OSError:
                    ...
                    return (time.time() - stat.st_mtime) < 300  # Less than 5 minutes old
            ...
            for filename in os.listdir(temp_dir):
                if filename.startswith("ray_port_") and filename.endswith(".lock"):
                    ...
                    if not self._is_lock_active(lock_file_path):
                        ...
                        LoggingUtility.log_info(
                            "port_allocation",
                            f"Cleaned up stale lock file {filename}"
                        )
        except (OSError, IOError) as e:
            LoggingUtility.log_warning(
