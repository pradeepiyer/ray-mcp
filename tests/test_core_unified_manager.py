...
class TestRayUnifiedManagerCore:
    ...
    def test_manager_instantiation_creates_all_components(
        self, mock_log_mgr, mock_job_mgr, mock_cluster_mgr, mock_port_mgr, mock_state_mgr
    ):
        ...
        mock_cluster_mgr.assert_called_once_with(mock_state_instance, mock_port_instance)
    ...
    def test_property_delegation_to_state_manager(self):
        ...
        with patch.object(manager._state_manager, 'get_state', return_value=mock_state):
            with patch.object(manager._state_manager, 'is_initialized', return_value=True):
...
class TestRayUnifiedManagerDelegation:
    ...
    async def test_cluster_management_delegation(self):
        ...
        mock_cluster_manager.init_cluster.assert_called_with(num_cpus=4, worker_nodes=[])
    ...
    async def test_job_management_delegation(self):
        ...
        mock_job_manager.submit_job = AsyncMock(return_value={"status": "success", "job_id": "job_123"})
        mock_job_manager.list_jobs = AsyncMock(return_value={"status": "success", "jobs": []})
        mock_job_manager.cancel_job = AsyncMock(return_value={"status": "success", "cancelled": True})
        mock_job_manager.inspect_job = AsyncMock(return_value={"status": "success", "job_info": {}})
        
        manager._job_manager = mock_job_manager
        
        # Test submit_job delegation
        result = await manager.submit_job("python script.py", runtime_env={"pip": ["numpy"]})
    ...
    async def test_log_management_delegation(self):
        ...
        mock_log_manager.retrieve_logs = AsyncMock(return_value={"status": "success", "logs": "Log content"})
        mock_log_manager.retrieve_logs_paginated = AsyncMock(return_value={
            "status": "success", 
            "logs": "Page content",
...
class TestRayUnifiedManagerBackwardCompatibility:
    ...
    async def test_method_signatures_compatible(self):
        ...
        manager._cluster_manager.init_cluster = AsyncMock(return_value={"status": "success"})
        ...
        manager._cluster_manager.stop_cluster = AsyncMock(return_value={"status": "success"})
        manager._cluster_manager.inspect_cluster = AsyncMock(return_value={"status": "success"})
        manager._log_manager.retrieve_logs = AsyncMock(return_value={"status": "success"})
        manager._log_manager.retrieve_logs_paginated = AsyncMock(return_value={"status": "success"})
...
class TestRayUnifiedManagerErrorPropagation:
    ...
    async def test_cluster_manager_error_propagation(self):
        ...
        mock_cluster_manager = Mock()
        mock_cluster_manager.init_cluster = AsyncMock(side_effect=Exception("Cluster initialization failed"))
        manager._cluster_manager = mock_cluster_manager
    ...
    async def test_job_manager_error_propagation(self):
        ...
        mock_job_manager = Mock()
        mock_job_manager.submit_job = AsyncMock(return_value={"status": "error", "message": "Job submission failed"})
        manager._job_manager = mock_job_manager
    ...
    async def test_log_manager_error_propagation(self):
        ...
        mock_log_manager = Mock()
        mock_log_manager.retrieve_logs = AsyncMock(return_value={"status": "error", "message": "Log retrieval failed"})
        manager._log_manager = mock_log_manager
    ...
    async def test_port_manager_error_propagation(self):
        ...
        mock_port_manager = Mock()
        mock_port_manager.find_free_port = AsyncMock(side_effect=RuntimeError("No free ports available"))
        manager._port_manager = mock_port_manager
...
class TestRayUnifiedManagerStateConsistency:
    ...
    def test_state_manager_shared_across_components(self):
        ...
        cluster_state_mgr = manager._cluster_manager.state_manager
        job_state_mgr = manager._job_manager.state_manager
        log_state_mgr = manager._log_manager.state_manager
        
        assert cluster_state_mgr is manager._state_manager
        assert job_state_mgr is manager._state_manager
        assert log_state_mgr is manager._state_manager
    ...
    def test_dependency_injection_consistency(self):
        ...
        assert manager._cluster_manager._state_manager is manager._state_manager
        assert manager._cluster_manager._port_manager is manager._port_manager
        
        # Other managers should have state manager
        assert manager._job_manager.state_manager is manager._state_manager
        assert manager._log_manager.state_manager is manager._state_manager

    @pytest.mark.parametrize("property_name", ["is_initialized", "cluster_address", "dashboard_url", "job_client"])
