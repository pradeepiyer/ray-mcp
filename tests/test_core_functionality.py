...
class TestFoundationComponents:
    ...
    def test_log_processor_stream_log_lines_memory_safety(self):
        """Test that _stream_log_lines handles memory exhaustion scenarios."""
        from ray_mcp.foundation.logging_utils import LogProcessor

        # Test with reasonable size limits
        short_lines = "line1\nline2\nline3"
        lines = list(
            LogProcessor._stream_log_lines(short_lines, max_line_size_bytes=100)
        )
        assert lines == ["line1", "line2", "line3"]

        # Test with extremely long line (should be truncated)
        long_line = "a" * 2000  # 2KB line
        log_with_long_line = f"normal_line\n{long_line}\nanother_line"
        lines = list(
            LogProcessor._stream_log_lines(log_with_long_line, max_line_size_bytes=1000)
        )

        assert len(lines) == 3
        assert lines[0] == "normal_line"
        assert lines[1].endswith("... (line truncated due to size limit)")
        assert len(lines[1]) < 1100  # Should be truncated plus marker
        assert lines[2] == "another_line"

        # Test with list input containing long lines
        long_lines = ["normal", "b" * 1500, "another"]
        lines = list(
            LogProcessor._stream_log_lines(long_lines, max_line_size_bytes=1000)
        )
        ...
        assert lines[1].endswith("... (line truncated due to size limit)")
        assert len(lines[1]) < 1100  # Should be truncated plus marker
        assert lines[2] == "another"

    def test_log_processor_stream_logs_with_limits(self):
        """Test stream_logs_with_limits functionality."""
        from ray_mcp.foundation.logging_utils import LogProcessor

        # Test normal case
        normal_logs = "line1\nline2\nline3\nline4\nline5"
        result = LogProcessor.stream_logs_with_limits(
            normal_logs, max_lines=3, max_size_mb=1
        )
        ...
        long_line = "x" * 2000
        logs_with_long_line = f"normal\n{long_line}\nafter"
        result = LogProcessor.stream_logs_with_limits(
            logs_with_long_line, max_lines=10, max_size_mb=1, max_line_size_kb=1
        )
    ...
    def test_log_processor_pagination_with_memory_limits(self):
        """Test stream_logs_with_pagination with memory limits."""
        import asyncio

        from ray_mcp.foundation.logging_utils import LogProcessor

        # Test pagination with long lines
        long_line = "y" * 1500
        logs = f"page1_line1\n{long_line}\npage1_line3\npage2_line1\npage2_line2"

        async def test_pagination():
            result = await LogProcessor.stream_logs_with_pagination(
                logs, page=1, page_size=3, max_size_mb=1, max_line_size_kb=1
            )
            ...
            assert log_lines[0] == "page1_line1"
            assert log_lines[1].endswith("... (line truncated due to size limit)")
            assert log_lines[2] == "page1_line3"

        asyncio.run(test_pagination())

    def test_log_processor_edge_cases(self):
        """Test edge cases for log processing."""
        from ray_mcp.foundation.logging_utils import LogProcessor

        # Test empty input
        result = LogProcessor.stream_logs_with_limits("", max_lines=10, max_size_mb=1)
        assert result == ""

        # Test single character per line
        single_chars = "a\nb\nc\nd"
        result = LogProcessor.stream_logs_with_limits(
            single_chars, max_lines=2, max_size_mb=1
        )
        lines = result.split("\n")
        assert len(lines) == 3  # 2 lines + truncation
        assert lines[0] == "a"
        assert lines[1] == "b"
        assert lines[2] == "... (truncated at 2 lines)"

        # Test no newlines (single very long line that exceeds limit)
        no_newlines = "z" * 2000  # 2KB line, exceeds 1KB limit
        result = LogProcessor.stream_logs_with_limits(
            no_newlines, max_lines=5, max_size_mb=1, max_line_size_kb=1
        )
        lines = result.split("\n")
        assert len(lines) == 1
        assert lines[0].endswith("... (line truncated due to size limit)")

        # Test unicode characters
        unicode_logs = "日本語\n中文\nالعربية"
        result = LogProcessor.stream_logs_with_limits(
            unicode_logs, max_lines=10, max_size_mb=1
        )
        lines = result.split("\n")
        assert lines[0] == "日本語"
        assert lines[1] == "中文"
        assert lines[2] == "العربية"
