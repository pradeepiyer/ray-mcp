...
class TestE2EIntegration:
    ...
    async def test_actor_management_workflow(self, ray_cluster_manager: RayManager):
        ...
        # Step 2: Create actors directly
        print("Creating actors directly...")

        # Create a simple actor script for testing
        actor_script_content = """
import ray
import time
...
if __name__ == "__main__":
    ...
    # Create actors
    actor1 = TestActor.remote("actor1")
    actor2 = TestActor.remote("actor2")
    
    # Test actor methods
    result1 = ray.get(actor1.increment.remote())
    result2 = ray.get(actor2.get_name.remote())
    
    print(f"Actor1 counter: {result1}")
    print(f"Actor2 name: {result2}")
    
    ray.shutdown()
"""
        # Write the actor script to a temporary file
        with tempfile.NamedTemporaryFile(mode="w", suffix=".py", delete=False) as f:
            f.write(actor_script_content)
            actor_script_path = f.name

        try:
            # Submit the actor job
            print("Submitting actor job...")
            ...
            assert (
                actor_job_data["status"] == "submitted"
            ), f"Expected 'submitted' status, got: {actor_job_data['status']}"
            actor_job_id = actor_job_data["job_id"]
            print(f"Actor job submitted with ID: {actor_job_id}")

            # Poll for actor job completion
            ...
            for i in range(max_wait):
                ...
                status_data = json.loads(status_content)
                job_status = status_data.get("job_status", "UNKNOWN")
                print(f"Actor job status at {i+1}s: {job_status}")
                if job_status == "SUCCEEDED":
                    break
                if job_status == "FAILED":
                    pytest.fail(f"Actor job failed unexpectedly: {status_data}")
                await asyncio.sleep(1)
            else:
                pytest.fail(
                    f"Actor job did not complete within {max_wait} seconds. Last status: {job_status}"
                )
            assert status_data["status"] == "success"
            assert status_data["job_status"] == "SUCCEEDED"
            print("Actor job completed successfully!")

            # Test actor listing
            print("Testing actor listing...")
            ...
            print(f"Found {len(actors_data['actors'])} actors in the cluster")

            # Test actor killing (if actors exist)
            if actors_data["actors"]:
                actor_id = actors_data["actors"][0]["actor_id"]
                print(f"Testing actor killing for actor {actor_id}...")
                kill_result = await call_tool("kill_actor", {"actor_id": actor_id})
                kill_content = get_text_content(kill_result)
                kill_data = json.loads(kill_content)
                assert kill_data["status"] == "success"
                print("Actor killed successfully!")

            print("Actor management tests completed!")

        finally:
            # Clean up actor script
            if os.path.exists(actor_script_path):
                os.unlink(actor_script_path)

        # Step 3: Stop Ray cluster
        print("Stopping Ray cluster...")
        stop_result = await call_tool("stop_ray")
        stop_content = get_text_content(stop_result)
        stop_data = json.loads(stop_content)
