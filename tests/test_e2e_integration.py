...
class TestE2EIntegration:
    ...
    async def test_port_allocation_race_condition_fix(self):
        ...
        if len(unique_ports) != len(ports_found):
            pytest.fail(
                f"RACE CONDITION! find_free_port() returned duplicate ports: {ports_found}"
            )

        print(
            f"✅ Concurrent allocation: {len(unique_ports)} unique ports in {allocation_time:.2f}s"
        )
        ...
        if len(unique_sequential) != len(sequential_ports):
            pytest.fail(
                f"RACE CONDITION! Sequential calls returned duplicate ports: {sequential_ports}"
            )

        print(
            f"✅ Sequential allocation: {len(unique_sequential)} unique ports in {sequential_time:.2f}s"
        )
        ...
        print(
            f"✅ Lock files before: {len(lock_files_before)}, after: {len(lock_files_after)}"
        )
    ...
    async def test_multiple_ray_clusters_unique_ports(self):
        ...
        try:
            ...
            async def start_cluster_task(manager, cluster_id, temp_dir):
                ...
                try:
                    ...
                    if result["status"] == "success":
                        ...
                        print(
                            f"✅ Cluster {cluster_id}: started on head port {head_port}, dashboard port {dashboard_port} in {startup_time:.2f}s"
                        )
                        return {
            ...
            tasks = [start_cluster_task(managers[i], i, temp_dirs[i]) for i in range(3)]
            ...
            for result in cluster_results:
                if isinstance(result, Exception):
                    ...
                    failed_clusters.append(
                        f"Cluster {result['cluster_id']}: {result['error']}"
                    )
                    ...
                    failed_clusters.append(f"Unexpected result type: {type(result)}")
            ...
            if len(unique_head_ports) != len(head_ports):
                pytest.fail(
                    f"HEAD PORT CONFLICT! Multiple clusters got same head ports: {head_ports}"
                )

            if len(dashboard_ports) > 0 and len(unique_dashboard_ports) != len(
                dashboard_ports
            ):
                pytest.fail(
                    f"DASHBOARD PORT CONFLICT! Multiple clusters got same dashboard ports: {dashboard_ports}"
                )

            # Expect at least 2 clusters to start successfully (allowing for occasional failures)
            if len(successful_clusters) < 2:
                pytest.fail(
                    f"Too many cluster failures: {len(failed_clusters)}/3 failed. This suggests a systematic issue."
                )
