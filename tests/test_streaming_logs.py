...
class TestStreamingLogs:
    ...
    def test_stream_logs_with_limits_size_exceeded(self, ray_manager):
        ...
        result = ray_manager._stream_logs_with_limits(logs, max_lines=3000, max_size_mb=1)
    ...
    def test_stream_logs_with_limits_line_limit_exceeded(self, ray_manager):
        ...
        result = ray_manager._stream_logs_with_limits(logs, max_lines=10, max_size_mb=10)
    ...
    async def test_stream_logs_with_pagination(self, ray_manager):
        ...
        # Test first page
        result = await ray_manager._stream_logs_with_pagination(logs, page=1, page_size=20, max_size_mb=1)
        ...
        # Test last page
        result = await ray_manager._stream_logs_with_pagination(logs, page=5, page_size=20, max_size_mb=1)
        assert result["status"] == "success"
    ...
    async def test_stream_logs_with_pagination_invalid_page(self, ray_manager):
        ...
        result = await ray_manager._stream_logs_with_pagination(logs, page=10, page_size=20, max_size_mb=1)
        assert result["status"] == "error"
