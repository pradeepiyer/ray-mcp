...
class TestRayManager:
    ...
    @pytest.mark.asyncio
    async def test_list_actors_success(self):
        """Test listing actors successfully."""
        ray_manager = RayManager()
        ...
        with patch("ray_mcp.ray_manager.RAY_AVAILABLE", True):
            with patch("ray_mcp.ray_manager.ray") as mock_ray:
                ...
                with patch.object(ray_manager, "_ensure_initialized"):
                    ...
                    mock_actor_handle._actor_id.hex.return_value = "test_actor_id"
                    ...
                    assert result["actors"][0]["name"] == "test_actor"
                    assert result["actors"][0]["namespace"] == "default"
                    assert result["actors"][0]["actor_id"] == "test_actor_id"
                    assert result["actors"][0]["state"] == "ALIVE"
