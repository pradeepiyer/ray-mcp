...
class TestRayLogManagerJobLogs:
    ...
    async def test_retrieve_job_logs_with_error_analysis(self):
        ...
        mock_client = Mock()
        mock_client.get_job_logs.return_value = "INFO: Starting job\nERROR: Something failed\nINFO: Continuing"
        
        manager = RayLogManager(state_manager)
        
        with patch.object(manager, '_get_job_client', return_value=mock_client):
            result = await manager._retrieve_job_logs_unified("job_123", include_errors=True)
    ...
    async def test_retrieve_job_logs_paginated(self):
        ...
        with patch.object(manager, '_get_job_client', return_value=mock_client):
            with patch.object(manager._log_processor, 'stream_logs_with_pagination') as mock_paginate:
                ...
                result = await manager._retrieve_job_logs_paginated("job_123", page=2, page_size=2)
...
FAILED: Operation unsuccessful"""
        ...
        assert analysis["error_count"] == 5  # ERROR, Exception, CRITICAL, Traceback, FAILED
