...
from ray_mcp.main import list_tools, call_tool, server
...
class TestRayHandlers:
    ...
    async def test_handle_cluster_routes_to_unified_manager(self):
        ...
        self.unified_manager_mock.handle_cluster_request.return_value = expected_response
    ...
    async def test_handle_tool_call_with_exception_in_handler(self):
        ...
        self.unified_manager_mock.handle_cluster_request.side_effect = Exception("Test exception")
...
class TestMCPServerIntegration:
    ...
    async def test_tool_routing_consistency(self):
        ...
        with patch('ray_mcp.main.handlers.handle_cluster') as mock_handle_cluster, \
            ...
            # Verify responses
            cluster_data = json.loads(cluster_result[0].text)
            job_data = json.loads(job_result[0].text)
            cloud_data = json.loads(cloud_result[0].text)
...
class TestErrorHandling:
    ...
    async def test_memory_pressure_handling(self):
        ...
        self.unified_manager_mock.handle_cluster_request.side_effect = memory_intensive_handler
