...
class TestToolExtensions:
    ...
    def mock_ray_manager(self):
        ...
        async def mock_submit_job(entrypoint, runtime_env=None, job_id=None, metadata=None, **kwargs):
    ...
    @pytest.mark.asyncio
    async def test_init_ray_kubernetes_cluster_type(self, tool_registry, mock_ray_manager):
    ...
    async def test_init_ray_address_provided(self, tool_registry, mock_ray_manager):
        ...
        mock_ray_manager.init_cluster.assert_called_once_with(address="127.0.0.1:10001", cluster_type="kubernetes")
        assert result["status"] == "success"
    ...
    async def test_submit_job_local_type(self, tool_registry, mock_ray_manager):
        ...
        with patch('ray_mcp.tool_registry.inspect.signature') as mock_signature:
            ...
            mock_sig = Signature([
                ...
                Parameter('runtime_env', Parameter.POSITIONAL_OR_KEYWORD, default=None),
                Parameter('job_id', Parameter.POSITIONAL_OR_KEYWORD, default=None),
                Parameter('metadata', Parameter.POSITIONAL_OR_KEYWORD, default=None),
            ])
            mock_signature.return_value = mock_sig
            
            result = await tool_registry.execute_tool("submit_job", {
                "entrypoint": "python script.py",
                "job_type": "local"
            })
            
            # Should call submit_job on manager
            mock_ray_manager.submit_job.assert_called_once_with(entrypoint="python script.py")
            assert result["status"] == "success"
    ...
    async def test_job_type_auto_detection_local(self, tool_registry, mock_ray_manager):
        ...
        type(mock_ray_manager).is_kubernetes_connected = PropertyMock(return_value=False)
    ...
    @pytest.mark.asyncio
    async def test_job_type_auto_detection_kubernetes(self, tool_registry, mock_ray_manager):
        """Test job type auto-detection for kubernetes clusters."""
        # Setup manager state using PropertyMock
        type(mock_ray_manager).kuberay_clusters = PropertyMock(return_value={"default/test-cluster": {}})
    ...
    async def test_list_jobs_kubernetes_type(self, tool_registry, mock_ray_manager):
        ...
        mock_ray_manager.list_kuberay_jobs.assert_called_once_with(namespace="ray-system")
        assert result["status"] == "success"
    ...
    async def test_kuberay_cluster_tools(self, tool_registry, mock_ray_manager):
        ...
        result = await tool_registry.execute_tool("list_kuberay_clusters", {
            "namespace": "ray-system"
        })
        mock_ray_manager.list_kuberay_clusters.assert_called_once_with(namespace="ray-system")
        
        # Test inspect_kuberay_cluster
        mock_ray_manager.get_kuberay_cluster = AsyncMock(return_value={"status": "success"})
        result = await tool_registry.execute_tool("inspect_kuberay_cluster", {
            "cluster_name": "test-cluster",
            "namespace": "ray-system"
        })
        mock_ray_manager.get_kuberay_cluster.assert_called_once_with("test-cluster", "ray-system")
    ...
    async def test_kuberay_job_tools(self, tool_registry, mock_ray_manager):
        ...
        result = await tool_registry.execute_tool("list_kuberay_jobs", {
            "namespace": "ray-system"
        })
        mock_ray_manager.list_kuberay_jobs.assert_called_once_with(namespace="ray-system")
        ...
        result = await tool_registry.execute_tool("inspect_kuberay_job", {
            "job_name": "test-job",
            "namespace": "ray-system"
        })
        mock_ray_manager.get_kuberay_job.assert_called_once_with("test-job", "ray-system")
