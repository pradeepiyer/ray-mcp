        ...
        with patch("ray_mcp.ray_manager.RAY_AVAILABLE", True):
            with patch("ray_mcp.ray_manager.ray") as mock_ray:
                ...
                with patch.object(
                    ...
                    assert ("Shutdown failed" in result["message"] or 
                            "Worker stop failed" in result["message"])

    @pytest.mark.asyncio
    async def test_worker_node_resource_allocation(self, ray_manager, mock_cluster_startup):
                        ...
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                mock_start_workers.return_value = [
                                    {"status": "started", "node_name": "high-resource-worker"},
                        ...
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                with patch.object(
                                    ...
                                    for i, worker_result in enumerate(result["worker_nodes"]):
                                        assert worker_result["status"] == "started"
                                        assert worker_result["node_name"] == f"worker-{i+1}"
