...
class TestWorkerManager:
    ...
    @pytest.mark.asyncio
    async def test_stop_all_workers_zombie_prevention_waitpid_success(
        self, worker_manager
    ):
        """Test zombie prevention using waitpid when process.wait() times out."""
        mock_process = Mock()
        mock_process.pid = 12345
        mock_process.poll.return_value = None  # Process is running
        mock_process.terminate.return_value = None
        mock_process.kill.return_value = None
        # Graceful wait times out, force kill wait times out, but waitpid succeeds
        mock_process.wait.side_effect = [
            subprocess.TimeoutExpired(cmd="ray start", timeout=5),  # Graceful timeout
            subprocess.TimeoutExpired(cmd="ray start", timeout=3),  # Force kill timeout
        ]

        worker_manager.worker_processes = [mock_process]
        worker_manager.worker_configs = [{"node_name": "worker-1"}]

        # Mock os.waitpid to return success on first attempt
        with patch("os.waitpid", return_value=(12345, 0)) as mock_waitpid:
            result = await worker_manager.stop_all_workers()

            # Verify waitpid was called
            mock_waitpid.assert_called()

        assert len(result) == 1
        assert result[0]["status"] == "force_stopped"
        assert result[0]["node_name"] == "worker-1"
        assert result[0]["message"] == "Worker node 'worker-1' force stopped"
        assert result[0]["process_id"] == 12345
    ...
    @pytest.mark.asyncio
    async def test_stop_all_workers_zombie_prevention_process_already_dead(
        self, worker_manager
    ):
        """Test zombie prevention when process is already dead."""
        mock_process = Mock()
        mock_process.pid = 12345
        mock_process.poll.return_value = None  # Process is running
        mock_process.terminate.return_value = None
        ...
        mock_process.wait.side_effect = [
            subprocess.TimeoutExpired(cmd="ray start", timeout=5),
            subprocess.TimeoutExpired(cmd="ray start", timeout=3),
        ]

        worker_manager.worker_processes = [mock_process]
        worker_manager.worker_configs = [{"node_name": "worker-1"}]

        # Mock os.waitpid to return ESRCH (no such process) indicating already dead
        def waitpid_side_effect(pid, flags):
            raise OSError(3, "No such process")  # ESRCH

        # Mock os.kill to also indicate process is dead
        def kill_side_effect(pid, signal):
            raise OSError(3, "No such process")  # ESRCH

        with patch("os.waitpid", side_effect=waitpid_side_effect):
